class PokemonRegionMap_Scene  
  ...
  def getWildInfo(map, gameMap)
    return 0, 0, 0, 0, "" if !ARMSettings::PROGRESS_COUNT_POKEMON
  ...
  def getTrainerInfo(map, district)
    return 0, 0, 0, "" if !ARMSettings::PROGRESS_COUNT_TRAINERS
  ...
  def getItemInfo(map, district)
    return 0, 0, 0, "" if !ARMSettings::PROGRESS_COUNT_ITEMS
  ...
  def getEncIcons
    ...
    @list.each do |species|
      if seenFormAnyGender(mapID, species, GameData::Species.get(species).form, @revealAllSeen)
        seen << species
      else
        unseen << species
      end
  ...
  def updateEncCursor(index)
    ...
    @sprites["ExtCursor"].x = @encSprites[index].x - 7
    @sprites["ExtCursor"].y = @encSprites[index].y - 7
  end 
